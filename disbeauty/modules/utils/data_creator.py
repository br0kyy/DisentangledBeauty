...
# Removed disbeauty.configs import
BASE_PATH = ""
...
for i in tqdm(range(NUMBER_OF_IMAGES // num_of_images_in_single_loop)):
    ...
    for index in range(len(sample)):
        if (counter % 1000) == 0:
            Path(f"{W_DATA_DIR}{int(counter / 1000)}").mkdir(
                parents=True, exist_ok=True
            )
            Path(f"{IMAGE_DATA_DIR}{int(counter / 1000)}").mkdir(
                parents=True, exist_ok=True
            )
            cur_dir = int(counter / 1000)

        with open(f"{W_DATA_DIR}{cur_dir}/{counter}.npy", "wb") as f:
            np.save(f, latents[index][0].cpu().numpy())
